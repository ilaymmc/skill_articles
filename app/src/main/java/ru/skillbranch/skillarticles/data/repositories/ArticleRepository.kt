...
class CommentsDataSource(
    ...
    override fun loadInitial(
        ...
        Log.e(
            "ArticleRepository",
            "loadInitial: start = ${params.requestedInitialKey}, size = ${params.requestedLoadSize}" +
                    ", resultSize = ${result.size}, totalCount = $totalCount")
        callback.onResult(
            if (totalCount > 0) {
                result
            } else {
    ...
    override fun loadAfter(params: LoadParams<String>, callback: LoadCallback<CommentItemData>) {
        ...
        Log.e(
            "ArticleRepository",
            "loadAfter: start = ${params.key}, size = ${params.requestedLoadSize}, resultSize = ${result.size}")
