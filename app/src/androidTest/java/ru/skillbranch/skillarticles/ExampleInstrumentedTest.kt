...
class ExampleInstrumentedTest {
    ...
    fun module2() {
        ...
        scenario.onActivity { activity ->
            val scroll = activity.findViewById<NestedScrollView>(
                R.id.scroll
            )
    ...
    fun module4() {
        ...
        Espresso.onView(
            ...
            .check(
                ViewAssertions.matches(
                    ViewMatchers.withText("test content")
                )
            )
        Espresso.onView(
            CoreMatchers.allOf(
        ...
        Espresso.onView(
            CoreMatchers.allOf(
                ...
                ViewMatchers.withParentIndex(1)
            )
        )
            .check(
                ViewAssertions.matches(
                    ViewMatchers.withText("test category")
                )
            )
        ...
        Espresso.onView(
            ...
            .check(
                ViewAssertions.matches(
                    ViewMatchers.withText("Not found")
                )
            )
        Espresso.onView(
            ViewMatchers.withId(
    ...
    fun module5() {
        ...
        scenario.onActivity { activity ->
            activity.binding.bind(
                ArticleState().copy(content = listOf(content)))
    ...
    private fun withFontSize(expectedSize: Float): Matcher<View> {
        return object : BoundedMatcher<View, View>(
            TextView::class.java) {
